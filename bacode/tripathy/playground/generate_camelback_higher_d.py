...
def sample_projection_matrices():
    real_dim = 2
    for i in range(2, 5):
        print("\n\n\nProjecting from " + str(i) + " to " + str(real_dim))
        A = sample_orthogonal_matrix(real_dim=i, active_dim=2).T
        assert A.shape == (real_dim, i), ("Something went wrong!", (A.shape), (real_dim, i))

        new_domain = np.dot(domain, A)
        print("Matrix: ")
        print(A)
        print("Domain")
        print(new_domain)
...
if __name__ == "__main__":
    print("Printing multiple orthogonal matrices")
    sample_projection_matrices()
