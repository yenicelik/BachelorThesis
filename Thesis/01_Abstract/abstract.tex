...
Most of these techniques concentrate on doing one of two things.
The most common approach is to make a linear dimensionality reduction.
...
This feature ultimately allows BORING to be more accurate when modeling functions that we want to optimize over. \\
...
In addition to that, we do a short experiment on whether the subspace identification method can be used to make feature selection, by choosing the projection in such a way, that the most important features will receive the highest matrix weights. \\
...
The main features of BORING are 1.) the possibility to identify the subspace (unlike most other optimization algorithms), and 2.) to provide a much lower penalty to identify the subspace, as optimization is still the primary goal.
